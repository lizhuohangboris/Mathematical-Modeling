...
# Neural Network Model: A simple multi-layer perceptron (MLP)
def build_model():
    model = tf.keras.Sequential([
        tf.keras.layers.Dense(64, activation='relu', input_dim=X_scaled.shape[1]),
        tf.keras.layers.Dense(32, activation='relu'),
        tf.keras.layers.Dense(1)  # Output layer with 1 neuron (since we predict a single value)
    ])
...
for target, y in zip(['Market_Size', 'Unit_Price', 'Online_Share', 'Offline_Share', 'Growth_Percentage', 'Average_Price'],
                      [y_market_size, y_unit_price, y_online_share, y_offline_share, y_growth_percentage, y_average_price]):
    model = build_model()
