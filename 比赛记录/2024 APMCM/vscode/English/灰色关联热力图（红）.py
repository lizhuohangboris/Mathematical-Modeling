...
# 设置字体支持
rcParams['font.sans-serif'] = ['Arial']  # 使用英文字体
...
# 设置因变量和自变量
dependent_variables = ["Cat", "Dog", "Market Size"]
...
def gray_relation_analysis(reference_cols, comparison_data):
    ...
    gray_relation_results.rename(columns={"index": "Variable"}, inplace=True)
...
# 打印结果
print("Gray Relation Analysis Results:")
print(gray_relation_results)

# 将结果转换为适合热力图的数据格式
heatmap_data = gray_relation_results.set_index("Variable")
...
sns.heatmap(
    ...
    cmap="Reds",  # 设置红色渐变色
    linewidths=0.5,
    # cbar_kws={'label': 'Gray Relation Degree'}
)
plt.title(
    " ", 
    fontsize=22, 
...
print(f"Gray relation analysis results saved to {output_path}")
