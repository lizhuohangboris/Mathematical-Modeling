...
class_weights = dict(zip(range(2), df[target].value_counts(normalize=True)))
...
class_weights_xg = df[target].value_counts(normalize=True).to_dict()

# Train XGBoost model with class weights
xg_model = XGBClassifier(random_state=42, scale_pos_weight=class_weights_xg[0]/class_weights_xg[1])
